...
class GitCli {
  ...
  diffFromParentSha(file, sha) {
    return asString(cp.execSync("git diff " + sha + "^! " + path.join(this.basedir(), file)));
  }
  ...
  removeFile(path) {
    return cp.execSync("rm -rf " + this.filepath(path));
  }

  log() {
    var lines = asString(cp.execSync("git log --pretty=format:\"%H|%ad|%s%d [%an]\" --date=short")).split("\n");
    var logs = [];

    lines.forEach((line) => {

      var i = line.indexOf("|");
      var sha = line.substring(0, i);
